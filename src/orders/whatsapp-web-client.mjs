...
export class WhatsAppWebClient {
 ...
 async fail(message,requiresScan=false){if(this.closed||this.failing)return;this.failing=true;this.generation++;clearTimeout(this.networkTimer);this.networkTimer=null;clearInterval(this.watchdog);this.mediaQueue=[];this.stateChange({status:requiresScan?'auth-required':'reconnecting',message,qr:null});const client=this.client;this.client=null;await this.destroy(client);this.connecting=null;this.connectAttempt=null;this.failing=false;if(!requiresScan&&!this.closed){clearTimeout(this.retryTimer);this.retryTimer=setTimeout(()=>{void this.connect();},this.delays[Math.min(this.attempt++,this.delays.length-1)]);this.retryTimer.unref?.();}}
 async destroy(client){if(!client)return;client.kdocsClosing=true;client.pupPage?.removeAllListeners?.('framenavigated');try{await bounded(client.destroy(),10000);}catch{try{client.pupBrowser?.process()?.kill('SIGTERM');}catch{}}finally{this.ownedClients.delete(client);}}
