...
async function bootstrap(){
  ...
  let lastWhatsAppStatus;whatsappBrowser.subscribe?.(e=>{if(e.kind==='state'&&e.state.status!==lastWhatsAppStatus){lastWhatsAppStatus=e.state.status;lifecycle('WhatsApp自动连接',e.state.message,{status:e.state.status,provider:e.state.provider,hasQR:Boolean(e.state.qr)});}});
