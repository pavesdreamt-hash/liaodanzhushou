...
import {translateManualReply,generateManualAssistantDraft,recognizeImageText,ChatTranslationCache} from './manual-translation.mjs';
...
import {mkdtemp,cp,writeFile,readFile,unlink,mkdir} from 'node:fs/promises';
...
import {validateExtraction} from './orders/order-assistant.mjs';
...
import {AddressDeliveryVerifier,loadAddressVerificationConfiguration,normalizeDeliveryRange} from './orders/address-delivery-verification.mjs';
...
function createTray(){if(tray)return tray;const openFromTray=()=>openDashboard().catch(error=>nativeFailure('无法打开管理页面',error)),quitFromTray=()=>{quitting=true;lifecycle('lifecycle','收到菜单栏退出请求');app.quit();};const source=nativeImage.createFromPath(path.resolve(app.getAppPath(),'assets/app-icon-1024.png'));if(source.isEmpty())throw new Error('菜单栏图标无法加载');const icon=source.resize({width:18,height:18,quality:'best'});tray=new Tray(icon);tray.setToolTip(DISPLAY_NAME);tray.setContextMenu(Menu.buildFromTemplate([{label:'打开管理页面',click:openFromTray},{type:'separator'},{label:'退出',click:quitFromTray}]));tray.on('click',openFromTray);if(testMode)Object.defineProperty(app,'__liaodanTrayTest',{configurable:true,value:Object.freeze({labels:Object.freeze(['打开管理页面','退出']),invoke:label=>{if(label==='打开管理页面')return openFromTray();if(label==='退出')return quitFromTray();throw new Error('未知菜单栏测试操作');}})});return tray;}
let assistantConnectors={},assistantSettings,shopPlusConnection,whatsappBrowser;
...
async function createWindow(dashboardUrl=null){
  ...
  window.on('close',event=>{if(quitting)return;event.preventDefault();const target=window;suppressCloseActivation=true;clearTimeout(suppressCloseActivationTimer);suppressCloseActivationTimer=setTimeout(()=>{suppressCloseActivation=false;},500);setTimeout(()=>{if(quitting||window!==target||target.isDestroyed())return;target.hide();lifecycle('window','主窗口已隐藏至菜单栏',{visible:target.isVisible()});void target.webContents.executeJavaScript('window.flushAssistantWorkspaces?.()').catch(error=>lifecycle('window','窗口隐藏后的工作区保存暂未完成',{message:error.message}));},100);});
  ...
  let rendererRecoveryTimer,rendererRecoveryAttempts=[];
  const recoverRenderer=details=>{
    if(quitting||!window||window.isDestroyed())return;
    const now=Date.now();rendererRecoveryAttempts=rendererRecoveryAttempts.filter(time=>now-time<60000);
    if(rendererRecoveryAttempts.length>=2){lifecycle('renderer','渲染进程连续退出，已暂停自动恢复',{...details,attempts:rendererRecoveryAttempts.length});dialog.showErrorBox('聊单助手界面连续异常退出','已连续两次自动恢复失败。请从菜单栏图标选择“退出”后重新打开，并导出诊断报告。');return;}
    rendererRecoveryAttempts.push(now);const attempt=rendererRecoveryAttempts.length;
    lifecycle('renderer','将在后台自动恢复界面',{...details,attempt});
    clearTimeout(rendererRecoveryTimer);rendererRecoveryTimer=setTimeout(()=>{if(quitting||!window||window.isDestroyed())return;window.loadURL(dashboardUrl||webServer?.url||rendererUrl.href).then(()=>revealWindow()).catch(error=>nativeFailure('聊单助手界面恢复失败',error));},1000);
  };
  window.webContents.on('render-process-gone',(_event,details)=>{lifecycle('renderer','渲染进程退出',details);recoverRenderer(details);});
...
async function bootstrap(){
  ...
  const handleManualReplyTranslation=async request=>{
    if(!assistantSettings)return {ok:false,error:'无法访问翻译服务'};
    try{
      ...
      const recognizeMessageImage=async()=>{
        if(typeof payload?.messageId!=='string'||!payload.messageId||payload.messageId.length>200)throw new Error('图片消息标识无效');
        const row=await manualChat?.media({token:payload.token,messageId:payload.messageId});
        const dataUrl=row?.metadata?.media?.find(item=>item.status==='cached'&&typeof item.dataUrl==='string')?.dataUrl;
        if(!dataUrl)throw new Error('图片尚未读取完成，请等图片显示后重试');
        return recognizeImageText(assistantSettings,{dataUrl});
      };
      const draftAssistantReply=async()=>{
        if(typeof payload?.token!=='string'||!payload.token)throw new Error('请先关联并核对客户聊天');
        if(!manualChat)throw new Error('请先关联并核对客户聊天');
        const bound=await manualChat.target(payload.token),page=await manualChat.recent({token:payload.token,limit:20});
        const messages=page.messages.filter(message=>typeof message?.id==='string'&&typeof message?.text==='string'&&['customer','merchant'].includes(message.direction)&&typeof message?.sentAt==='string'&&message.text.trim()).map(({id,direction,text,sentAt})=>({id,direction,text,sentAt}));
        if(!messages.length)throw new Error('当前会话没有可用于 AI 草稿的文字消息');
        // The renderer supplies only intent/reference; the bounded conversation comes from
        // this still-valid binding, so a different chat cannot be injected into the request.
  ...
  ipcMain.handle('manual-reply-translation',async(event,request)=>{
    if(!window||window.isDestroyed()||event.sender!==window.webContents||event.senderFrame!==window.webContents.mainFrame)return {ok:false,error:'无法访问翻译服务'};
    return handleManualReplyTranslation(request);
  });
  ...
  const handleManualChat=async request=>{
    if(!whatsappBrowser||!paths)return {ok:false,error:'聊天服务尚未准备好'};
    try{
      ...
      const extractCustomer=async()=>{const {orderId,detail}=orderBinding(),bound=await manualChat.target(payload.token),page=await manualChat.recent({token:payload.token,limit:100});verifyOrderPhone(orderId,detail,bound.chatId);const rows=page.messages.map(message=>({id:message.id,direction:message.direction,text:message.text,sentAt:message.sentAt}));if(!rows.length)throw new Error('当前会话没有可识别的文字消息');const extracted=await assistantSettings.extract({messages:rows}),validated=validateExtraction(extracted,rows),labels={fullName:'客户姓名',phone:'联系电话',country:'国家/地区',province:'省/州',city:'城市',street:'街道',residence:'详细地址'},current={fullName:detail.customer.fullName||'',phone:detail.customer.phone||'',country:detail.customer.country||'',province:detail.customer.province||'',city:detail.customer.city||'',street:detail.customer.street||'',residence:detail.customer.residence||''},suggestions=Object.entries(validated.fields).filter(([field,claim])=>Object.hasOwn(labels,field)&&current[field]!==claim.value).map(([field,claim])=>({id:randomUUID(),field,label:labels[field],current:current[field]||'未填写',proposed:claim.value,evidence:claim.evidence,source:'ai'}));customerSuggestions.set(payload.token,{orderId,accountId:bound.accountId,chatId:bound.chatId,suggestions});return {suggestions,issues:validated.issues};};
      const applyCustomerSuggestion=async()=>{const bound=await manualChat.target(payload.token),stored=customerSuggestions.get(payload.token),suggestion=stored?.suggestions.find(item=>item.id===payload.suggestionId);if(!suggestion||stored.orderId!==payload.orderId||stored.accountId!==bound.accountId||stored.chatId!==bound.chatId)throw new Error('AI 建议已失效，请重新识别当前会话');const source=suggestion.evidence.map(item=>item.id).join('、').slice(0,220),detail=await orders().applyChatSuggestion({orderId:stored.orderId,field:suggestion.field,value:suggestion.proposed,reason:`AI聊天建议已人工确认；来源消息：${source}`});stored.suggestions=stored.suggestions.filter(item=>item.id!==suggestion.id);return {detail,appliedId:suggestion.id};};
      const inbox=async()=>{if(typeof whatsappBrowser.inbox!=='function')throw new Error('当前 WhatsApp 连接不支持统一会话列表');const hiddenChatIds=await manualChat.hiddenChats(manualChat.status().accountId);const page=await whatsappBrowser.inbox({...payload,hiddenChatIds,hiddenOnly:payload.hiddenOnly===true});return {...page,hiddenChatCount:hiddenChatIds.length};};
      const operations={status:()=>manualChat.status(),inbox,setInboxHidden:()=>manualChat.setInboxHidden(payload),openInbox:()=>manualChat.openInbox(payload),recent:()=>manualChat.recent(payload),media:()=>manualChat.media(payload),connect:()=>manualChat.connect(),inspect:()=>manualChat.inspect(payload),bind:()=>payload.orderId?bindOrder():manualChat.bind(payload),restore:restoreOrder,history:historyOrder,send:sendOrder,extractCustomer,applyCustomerSuggestion};
  ...
  ipcMain.handle('manual-chat',async(event,request)=>{
    ...
    return handleManualChat(request);
  });
  operations.set('manual-chat',handleManualChat);
  let manualImages;
  ...
  const fixtureUsesMatchingIsolatedData=Boolean(settingsFixture&&isolatedUserData&&path.resolve(isolatedUserData.slice('--isolated-user-data='.length))===path.resolve(testUserData.slice('--orders-test-user-data='.length)));
  // Only a fixture with exactly matching isolated test data may bypass a macOS
  // Keychain prompt. Normal starts and non-isolated tests retain secure storage.
  const fixtureSecretStorage=fixtureUsesMatchingIsolatedData?{isEncryptionAvailable:()=>true,encryptString:value=>Buffer.from(value,'utf8'),decryptString:value=>Buffer.from(value).toString('utf8')}:null;
  ...
  // Keep verified direct chats even when no order is linked or the workbench is hidden.
  manualChat=new ManualChat({adapter:whatsappBrowser,directory:path.join(paths.base,'orders','manual-chat')});
  let lastWhatsAppStatus;whatsappBrowser.subscribe?.(e=>{if(e.kind==='orphaned-browser-reclaimed')lifecycle('WhatsApp自动连接','已清理上次异常退出遗留的 WhatsApp 浏览器进程，正在恢复连接',{count:e.count});if(e.kind==='state'&&e.state.status!==lastWhatsAppStatus){lastWhatsAppStatus=e.state.status;lifecycle('WhatsApp自动连接',e.state.message,{status:e.state.status,provider:e.state.provider,hasQR:Boolean(e.state.qr),failureCode:e.state.lastError?.code||null,failureMessage:e.state.lastError?.message||null});}});
  ...
  const shopPlusStatus=async()=>({...await shopPlusConnection.status(),sync:orders().shopPlusSyncStatus()});
  handler('orders:shopplus-status',shopPlusStatus);
  handler('orders:configure-shopplus',async()=>{const configured=await shopPlusConnection.configure();if(configured.canceled)return {...configured,state:{...configured.state,sync:orders().shopPlusSyncStatus()}};const state=await shopPlusConnection.verify();return {canceled:false,state:{...state,sync:orders().shopPlusSyncStatus()}};});
  handler('orders:sync-shopplus',async()=>{try{const sync=orders().shopPlusSyncStatus(),response=await shopPlusConnection.listLatestOrders({updatedAtMin:sync.lastSuccessfulAt||null});await shopPlusConnection.markVerified('已通过 ShopPlus 订单读取验证');const result=await orders().syncShopPlusApiOrders({orders:response.orders,fetchedAt:new Date().toISOString(),truncated:response.truncated});return {result,response,status:await shopPlusStatus()};}catch(error){await shopPlusConnection.markFailed(`同步失败：${String(error?.message||error)}`).catch(()=>{});throw error;}});
  ...
  handler('orders:confirm-order',payload=>orders().confirmOrder(payload));handler('orders:confirm-customer-information',payload=>orders().confirmCustomerInformation(payload));handler('orders:confirm-report-review',payload=>orders().confirmReportReview(payload));handler('orders:confirm-report-item',payload=>orders().confirmReportItem(payload));handler('orders:update-report-number',payload=>orders().updateReportNumber(payload));handler('orders:update-item-discount',payload=>orders().updateItemDiscount(payload));handler('orders:update-recipient',payload=>orders().updateRecipient(payload));handler('orders:restore-recipient',payload=>orders().restoreRecipient(payload));handler('orders:confirm-full-name',payload=>orders().confirmFullNameAsName(payload));handler('orders:update-package-status',payload=>orders().updatePackageStatus(payload));handler('orders:edit-order',payload=>orders().editOrder(payload));handler('orders:correct-order-status',payload=>orders().correctOrderStatus(payload));handler('orders:confirm-package-fee',payload=>orders().confirmPackageFee(payload));handler('orders:preview-package-txt',payload=>orders().previewPackageTxt(payload));handler('orders:profit-report',payload=>orders().profitReport(payload));
  ...
  handler('orders:decide-delivery-city',async payload=>{const orderId=String(payload?.orderId||''),decision=String(payload?.decision||'');if(!orderId||!['deliverable','out_of_range'].includes(decision))throw new Error('配送确认参数无效');const detail=orders().detail({id:orderId}),city=String(detail?.customer?.city||'').trim();if(!city)throw new Error('请先补全客户城市，再确认配送范围');const configuration=await loadAddressVerificationConfiguration(paths.base),range=normalizeDeliveryRange(configuration.range),normalizedCity=city.replace(/\s+/gu,' ').toLocaleLowerCase('en-US');if(decision==='deliverable'){range.cities=[...new Set([...range.cities,normalizedCity])];range.blockedCities=range.blockedCities.filter(value=>value!==normalizedCity);}else{range.blockedCities=[...new Set([...range.blockedCities,normalizedCity])];range.cities=range.cities.filter(value=>value!==normalizedCity);}await mkdir(path.dirname(configuration.rangeFile),{recursive:true});await writeFile(configuration.rangeFile,JSON.stringify(normalizeDeliveryRange(range),null,2),{encoding:'utf8',mode:0o600});addressVerifier.range=normalizeDeliveryRange(range);return orders().recordAddressVerification({orderId,status:decision==='deliverable'?'matched':'not_matched',detailConfirmed:decision==='deliverable'});});
...
const instanceLockAcquired=testMode||app.requestSingleInstanceLock();
if(!instanceLockAcquired)app.quit();else{
  ...
  app.on('activate',()=>{if(suppressCloseActivation){suppressCloseActivation=false;clearTimeout(suppressCloseActivationTimer);lifecycle('lifecycle','忽略关闭窗口引发的激活事件');return;}lifecycle('lifecycle','收到macOS激活事件');openDashboard().catch(error=>nativeFailure('无法打开管理页面',error));});let quitReady=false,quitSaving=false;app.on('before-quit',event=>{if(!quitReady){event.preventDefault();if(quitSaving)return;quitSaving=true;Promise.all([window&&!window.isDestroyed()?window.webContents.executeJavaScript('window.flushAssistantWorkspaces?.()'):Promise.resolve(),webServer?.flushAssistantWorkspaces()]).then(async()=>{await Promise.all([orderApp?.assistant.online.stop(),whatsappBrowser?.close()]);await manualChat?.captureQueue;}).then(()=>{quitReady=true;app.quit();}).catch(error=>{quitting=false;lifecycle('lifecycle','安全退出失败，已取消退出',{message:String(error?.message||error)});revealWindow();nativeFailure('退出未完成',error);}).finally(()=>{quitSaving=false;});return;}quitting=true;clearTimeout(suppressCloseActivationTimer);clearInterval(priceCheckTimer);void whatsappBrowser?.close().catch(()=>{});void clearExcelSelections();restoreSelections.clear();orderApp?.invalidatePreviews();webServer?.close();closeKdocsWorkspace().catch(error=>lifecycle('KDocs工作区','关闭浏览器工作区失败',{message:error.message}));if(orderDatabase){orderDatabase.close();orderDatabase=null;}});app.on('window-all-closed',()=>{if((process.env.NODE_ENV==='test'&&!testBrowserWorkspace)||quitting)app.quit();});
}
