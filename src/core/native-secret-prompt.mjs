...
export function secretPromptScript(label){
  ...
  return `text returned of (display dialog "请输入 ${label} API 密钥。保存后自动复用，不会显示在网页中。" default answer "" with hidden answer buttons {"取消", "保存"} default button "保存" cancel button "取消" with title "KDocs 助手设置")`;
}
